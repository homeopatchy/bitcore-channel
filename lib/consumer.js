...
Consumer.prototype.setupRefund = function() {
  ...
  var fee = this.commitmentTx.getFee();
  var amount = this.commitmentTx.amount - fee;
  var multisigOut = {
...
Consumer.prototype.validateRefund = function(refund) {
  ...
  $.checkState(amount === (this.commitmentTx.amount - this.commitmentTx.getFee()) - refund.getFee(),
  ...
  var multisigOut = {
    ...
    outputIndex: 0,
    satoshis: amount + refund.getFee(),
    script: this.commitmentTx.outputs[0].script
  };
  this.paymentTx = new Payment({
    multisigOut: multisigOut,
    amount: amount + refund.getFee(),
