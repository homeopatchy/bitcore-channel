...
function Consumer(opts) {
   ...
   */
  this.fundingAddress = this.fundingKey.toAddress(this.network);
 ...
 * @param {Object} utxo
 */
Consumer.prototype.processFunding = function(utxo, feePerKb) {
  $.checkArgument(_.isObject(utxo), 'Can only process an array of objects or an object');
  this.commitmentTx.from(utxo);
  var size = this.commitmentTx._estimateSize() / 1024;
  this.commitmentTx.fee((size * feePerKb) | 0);
};
 ...
 */
Consumer.prototype.setupRefund = function(feePerKb) {
  this.commitmentTx.sign(this.fundingKey);
  $.checkState(this.commitmentTx.isFullySigned());
  var commitmentFee = this.commitmentTx._getInputAmount() -
    this.commitmentTx._getOutputAmount();
  var amount = this.commitmentTx.amount - commitmentFee;
  var multisigOut = {
  ...
  var size = (this.refundTx._estimateSize() + 34) / 1024;
  var fee = (size * feePerKb) | 0;
  this.refundTx.to(this.refundAddress, this.refundTx.inputAmount - fee);
 ...
 */
Consumer.prototype.validateRefund = function(refund, feePerKb) {
  ...
  var fee = refund._getInputAmount() - refund._getOutputAmount();
  var amount = refund.outputs[0].satoshis;
  $.checkState(amount + fee === this.commitmentTx.amount - this.commitmentTx.getFee(),
  ...
  amount = this.commitmentTx.outputs[0].satoshis;
  var multisigOut = {
  ...
  var size = this.paymentTx._estimateSize() / 1024;
  this.paymentTx.fee((size * feePerKb) | 0)
 ...
 */
Consumer.prototype.incrementPaymentBy = function(satoshis, feePerKb) {
  this.paymentTx.updateValue(satoshis, feePerKb);
  this.paymentTx.sign(this.commitmentKey);
